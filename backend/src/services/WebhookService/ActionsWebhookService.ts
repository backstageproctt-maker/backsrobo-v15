...
export const ActionsWebhookService = async (
  ...
  try {
    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      // TRAVA DE SEGURANÇA: Verifica se o ticket foi fechado ou teve o bot desligado (ex: botão Parar Bot)
      if (idTicket) {
        const checkStatus = await Ticket.findOne({ where: { id: idTicket, companyId } });
        if (!checkStatus || checkStatus.hashFlowId === null || checkStatus.status === "closed") {
          console.log("Fluxo interrompido pelo usuário (hashFlowId nulo ou closed) -> Cancelando execução.");
          break;
        }
      }
      ...
      if (nodeSelected.type === "singleBlock") {
        for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
          ...
          // TRAVA DE SEGURANÇA INTERNA DO SINGLE BLOCK
          if (!ticket || ticket.hashFlowId === null || ticket.status === "closed") {
            console.log("Fluxo interrompido pelo usuário no meio de um bloco múltiplo -> Cancelando execução.");
            break;
          }
          ...
          // ---------- INÍCIO DO BLOCO ADICIONADO PARA TAGS ----------
          if (elementNowSelected.includes("tag")) {
            ...
            try {
              ...
              // Verifica se a tag já existe neste ticket para não duplicar
              const existTag = await TicketTag.findOne({ 
              ...
              if (!existTag) {
                ...
                // Emite evento via socket para atualizar o frontend em tempo real
              ...
              console.log("Erro ao adicionar tag pelo flowbuilder", err);
            }
          }
