...
useEffect(() => {
  ...
  const [laneLists, setLaneLists] = useState([]);
  ...
  useEffect(() => {
    if (isMounted.current) {
      ...
      api.get(`/tags/list`, { params: { companyId, kanban: 1 } })
        .then(({ data }) => {
          const formattedLaneLists = data
            .map((tag) => ({
              id: tag.id,
              name: tag.name,
            }));
          setLaneLists(formattedLaneLists);
        })
        .catch((error) => {
          console.error("Erro ao buscar Lanes:", error);
        });
                ...
                <Grid spacing={2} container>
                  ...
                  <Grid xs={12} md={4} item>
                    <FormControl
                      ...
                      className={classes.formControl}
                    >
                      <InputLabel id="laneId-selection-label">
                        Lane
                      </InputLabel>
                      <Field
                        as={Select}
